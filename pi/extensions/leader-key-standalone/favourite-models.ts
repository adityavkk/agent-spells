...
import { ALL_THINKING_LEVELS, getCurrentThinkingLevel, setCurrentThinkingLevel } from "./model-switcher.js";
...
function loadFavourites(): FavouriteModelEntry[] {
	...
	try {
		...
		return parsed.filter((e) => typeof e.label === "string" && typeof e.provider === "string" && typeof e.model === "string").slice(0, MAX_FAVOURITES);
	} catch {
		return [];
	}
}
...
export async function runFavouriteModels(pi: ExtensionAPI, ctx: ExtensionContext): Promise<void> {
	...
	const selected = await ctx.ui.custom<PickerResult | null>((tui, theme, _kb, done) => {
		...
		return {
			render: (width: number) => {
				...
				for (let i = 0; i < favourites.length; i++) {
					...
					const isCurrent = currentModel?.provider === fav.provider && currentModel?.id === fav.model;
					const label = isHighlighted ? th.fg("accent", th.bold(fav.label)) : th.fg("text", fav.label);
					...
					const thinkingTag = isHighlighted ? th.fg("dim", "‹") + th.fg(thinkingRole, ` ${thinking} `) + th.fg("dim", "›") : th.fg(thinkingRole, thinking);
					const num = th.fg("dim", `${i + 1}`);
					lines.push(f.rowTruncated(`${isHighlighted ? "> " : "  "}${num} ${label} ${providerTag}${currentBadge}  ${thinkingTag}`));
				}
				lines.push(f.separator());
			...
			handleInput: (data: string) => {
				if (matchesKey(data, "escape") || matchesKey(data, Key.ctrl("c")) || matchesKey(data, "backspace")) {
					done(null);
					return;
				}
				...
				if (matchesKey(data, "enter")) {
					done({ fav: favourites[highlightedIndex], thinking: ALL_THINKING_LEVELS[thinkingIndices[highlightedIndex]] });
				}
			},
		};
	}, {
	...
	setCurrentThinkingLevel(pi, ctx, selected.thinking);
